...
void PushDown(int k){
    ...
    tree[k*2].w += tree[k].lazy;
    tree[k*2+1].lazy += tree[k].lazy;
    tree[k*2+1].w += tree[k].lazy;
    //清空传递完的lazy
    tree[k].lazy = 0;
}
/*PushUp*/
...
![IMG_20191023_123055](README.assets/IMG_20191023_123055.jpg)

### 题解

* 从题面可直接看出这是一道线段树题，

* 仔细思考可以发现，**pot~p~(n)的值就是n可以最多分出几个p因子**，例如40 = 5*2^3^  ,pot~2~(40) = 3

* a~i~的初始值为1，之后反复乘以x（2<=x<=10),a的因子都是从x乘来的，所以素因子只有2~x=(2,4,6,8,10)~,3~x=(3,6,9)~,5~x=(5,10)~,7~x=(7)~

* 查询的值即为在[l,r]内4种素因子单点计数的最大值，所以只要用4棵线段树分别动态维护4种素因子的区间最大计数即可
